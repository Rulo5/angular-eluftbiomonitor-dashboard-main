...
`<ng-container *ngFor="let alert of outlineAlerts">
    <ngb-alert type="{{alert.type}}" (closed)="outlineClose(alert)">{{ alert.message }}</ngb-alert>
</ng-container>`
export let alertTS2 = 
`import { Component } from '@angular/core';
interface Alert {
...
`<ng-container *ngFor="let alert of solidAlerts">
    <ngb-alert type="{{alert.type}}" (closed)="solidClose(alert)">{{ alert.message }}</ngb-alert>
</ng-container>`
export let alertTS3 = 
`import { Component } from '@angular/core';
interface Alert {
...
`<ngb-alert type="warning">
    <span class="alert-inner--text"><strong>Holy guacamole!</strong> You should check in on some of those fields below.</span>
</ngb-alert>
